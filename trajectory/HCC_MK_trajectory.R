...
expr_mtx <- read_csv("./HCC_MK_adata_R/HCC_MK_adata.csv", col_names=FALSE)#expression matrix
expr_mtx <- t(as.matrix(expr_mtx))
sample_sheet <- read_csv("./HCC_MK_adata_R/obs.csv" ) #pheno data
row.names(sample_sheet) <- colnames(expr_mtx)
genes_ann <- as.data.frame(read_csv("./HCC_MK_adata_R/var.csv")) #feature data
...
HCC_MK <-orderCells(HCC_MK, root_state = GM_state(HCC_MK))
...
plot_genes_branched_heatmap(HCC_MK[row.names(subset(BEAM_res,
                                                    qval < 1e-4)),],
                            branch_point = 1,
                            num_clusters = 4,
                            cores = 1,
